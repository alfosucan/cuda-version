...
bool CudaFinder::swapVersion() {
  auto timer = Timer<std::chrono::seconds, void>(
      5, std::bind(&CudaFinder::on_time_out, this));
...
bool CudaFinder::is_lib64(const fs::path &path_) {
  if (!m_lib64_found &&
      ...
      !path_.has_extension() && fs::directory_entry(path_).is_directory()) {
    m_lib64_found = true;
...
bool CudaFinder::is_nvcc(const fs::path &path_) {
  if (!m_nvcc_found && path_.string().find("nvcc") != std::string::npos &&
      !path_.has_extension() && fs::directory_entry(path_).is_regular_file()) {
    m_nvcc_found = true;
